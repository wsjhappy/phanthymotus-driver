...
class LocoTests(unittest.TestCase):
    def setUp(self):
        ...
        self.card = loco_control.make_loco({}, '', None, self.client)
        self.card._poll = .003
    ...
    def test_completion_delivery_error_retained_without_blind_retries(self):
        ...
        with mock.patch('loco_control.urllib.request.urlopen', side_effect=TimeoutError('unknown delivery')) as post:
            loco_control.LocoPlugin._notify(self.card, job)
    ...
    def test_bundle_registers_replacement_loco_and_dispatches_alias(self):
        ...
        bundle = module.Go1Bundle({'plugins': {'loco': {'enabled': True}}}, 'go1', None, self.client)
        self.assertEqual([t['name'] for t in bundle.get_all_tools()], ['loco'])
        self.assertTrue(bundle.dispatch('loco', {'action': 'stop_move'})['stop_confirmed'])
