...
class Go1Bundle:
    ...
    def __init__(self, cfg, namespace, executor, client):
        ...
        if pc.get("loco_confirmed", {}).get("enabled", False):
            from loco_control import make_loco_confirmed
            self._plugins.append(make_loco_confirmed(pc["loco_confirmed"], namespace, executor, client))
            print("[bundle] loco_confirmed loaded (separate card, shared SDK)")
